...
// POST /api/payments — admin only (user hanya pesan, tidak bisa bayar)
r.post("/", roleMiddleware(["admin"]), paymentValidators, async (req, res) => {
...
r.get("/", roleMiddleware(["admin"]), async (req, res) => {
  try {
